...
public class DataSourceConfiguration {
    ...
    public DataSource clientDatasource() {
        ...
        DataSource clientDatasource = DataSourceBuilder.create().username("admin").password("admin").driverClassName("com.mysql.jdbc.Driver").url("jdbc:mysql://localhost/?useSSL=false&allowPublicKeyRetrieval=true").type(DriverManagerDataSource.class).build();
        DataSource clientDatasource_2 = DataSourceBuilder.create().username("admin").password("admin").driverClassName("com.mysql.jdbc.Driver").url("jdbc:mysql://localhost/SmartPiano?useSSL=false&allowPublicKeyRetrieval=true").type(DriverManagerDataSource.class).build();
        //We add them to the hashMap
        targetDataSources.put(AvaiableClients.adminGeneral, clientDatasource);
        targetDataSources.put(AvaiableClients.adminSmartPiano, clientDatasource_2);
